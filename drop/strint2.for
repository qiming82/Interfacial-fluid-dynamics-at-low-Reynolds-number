	...
	dimension dzg(20),drg(20),dsg(20)
...
c	   dsg(j) = sqrt(drg(j)**2+dzg(j)**2)
	end do
c----
c get the arclength first
c
c      arcl=0.0
c	do j=1,m
c        arcl=arcl + a*rg(j)*dsg(j)*w1(j)
c	enddo
