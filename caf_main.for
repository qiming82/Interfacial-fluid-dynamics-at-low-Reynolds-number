...
c      implicit real*8 (a-h,o-y)
	...
	dimension u_srrtmp2(neq),fc(neq)
	dimension en(neq),enp(neq),ipiv(neq)
...
c   prepare to store the data
      ...
      open (31,file='fcmax.dat')
      open (32,file='urmax.dat')
...
c---------------------------
       ...
       unm=maxval(un)
         ...
         fkmax = maxval(abs(fc(1:N+1)))
         v2max = maxval(dabs(u2))

c       call regridf2(znew,rnew,z,r,s,N,d,dt)
       call regridf(znew,rnew,z,r,s,N,v1max,dt)
...
c	   write(*,*) 'time=', time
	   ...
	   if((d-rmax).lt.0.05d0*a.and.dt.gt.dt2)then
	    dt=dt2
	   endif
	   if((d-rmax).lt.0.03d0*a.and.dt.gt.dt3)then
	    dt=dt3
	   endif
c	   if((d-rmax).lt.0.01d0*a.and.dt.gt.dt4)then
c	    dt=dt4
c	   endif
         ...
         if (rmin.lt.0.003D0*a) then
	       write(30,*) "neck too small, pinching"
	       write(30,*) 'rmin=', rmin
	       exit
	    end if
c         if (dabs(d-rmax).lt.0.005D0*a) then
c	       write(30,*) "amplitude too big, touching"
c	       write(30,*) 'd-rmax=', d-rmax
c	       exit
c	    end if
         jj = jj + 1
c	 write(*,*) jj
